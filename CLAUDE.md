...
3. **Browser/UI** — the AI JavaScript SDK is rendered into `src/WebApp/Components/App.razor` (page views with SPA route tracking, AJAX, JS exceptions) when the connection string is present. After a player joins, the client calls `farkleTelemetry.setPlayer` (interop in `Game.razor.cs`) to set the browser user/account.

**Dimensions (#216):** telemetry is sliceable by **game = `session.id`** and **player = `enduser.id`** (synthetic `g{gameId}-p{playerId}`, **no display names**). Server request spans get them from the route (`AspNetCoreTraceInstrumentationOptions.EnrichWithHttpResponse`); domain `customEvents` get them from `DomainEventLogProcessor` (mapping the `GameId`/`PlayerId` log attributes); the browser sets user/account via `setAuthenticatedUserContext`.
