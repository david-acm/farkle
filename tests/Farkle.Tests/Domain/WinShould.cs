...
public class WinShould : GameWithThreePlayersTest
{
  ...
  [Fact]
  public void EmitGameWonWhenPlayerReaches10000Points()
  {
    // Arrange — inject past-turn events to bring player 1's committed score to 9,700
    ...
    events.Add(new TurnPassed(p1.Id, ImmutableArray.Create(p2, p3, p1), 9_700));
    ...
    // Roll three 5s (trips = 500 pts) to push total over 10,000
  ...
  [Fact]
  public void NotEmitGameWonWhenScoreIsBelow10000()
  {
  ...
  [Fact]
  public void EmitGameWonWithScoreOfExactly10000WhenPlayerReachesThreshold()
  {
    // Arrange — bring player 1's committed score to exactly 9,900 and rotate
    ...
    var events = Game.Current.ToList();
    events.Add(new TurnPassed(p1.Id, ImmutableArray.Create(p2, p3, p1), 9_900));
    events.Add(new TurnPassed(p2.Id, ImmutableArray.Create(p3, p1, p2), 0));
    events.Add(new TurnPassed(p3.Id, ImmutableArray.Create(p1, p2, p3), 0));
    Game.Load(events);

    // Roll two 5s (100 pts) so the total lands on exactly 10,000
    SetupDiceToRoll(new List<int> { 5, 5, 2, 3, 4, 6 });
    Game.RollDiceV2(new RollDice(1, p1.Id));
    Game.KeepDice(new KeepDice(1, p1.Id, new[] { Five, Five }));
    Game.PassTurn(new PassTurn(1, p1.Id));

    // Assert — the >= 10,000 threshold fires at the exact-equality boundary
    var won = Changes.Should().ContainSingleEvent<GameWon>();
    won!.PlayerId.Should().Be(p1.Id);
    won.Score.Should().Be(10_000);
    State.GameStage.Should().Be(GameStage.Finished);
    State.Winner!.Id.Should().Be(p1.Id);
  }
