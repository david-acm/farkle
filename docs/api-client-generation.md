...
`src/WebApp.Client/swagger.json` and `src/HotDice.ApiClient/**` are **generated —
never hand-edit them**. Regenerate and commit after any change to the HTTP
contract (`src/HotDice.Contracts/HttpRequests.cs` / `HttpResponses.cs`). CI's
...
and `HotDice.WebTests`, so there's one regeneration command for all consumers.

## Steps

1. Edit the DTO in `src/HotDice.Contracts/HttpResponses.cs` (or `HttpRequests.cs`).
...
3. Regenerate the Kiota client (clean first so a removed path/schema can't leave an orphan):
   ```bash
   dotnet tool restore
   cd src/HotDice.ApiClient && rm -rf Api Models && dotnet kiota generate \
     -l CSharp -d ../WebApp.Client/swagger.json \
     -c HotDiceApiClient -n HotDice.ApiClient -o . \
     && cd -
   ```
4. Commit the regenerated `swagger.json` + `HotDice.ApiClient/`.
